...
int main(int argc, char* argv[]) {
  ...
  // Group 1: backpressure bookkeeping
  const int kNumA = 5;
  ...
  // Group 2: cross-thread index handling
  const int kNumB = 5;
  ...
  // Simulate churn and stale capture for B
  for (int i=0;i<kNumB;++i) {
    CountDownLatch idxCaptured(1);
    CountDownLatch proceed(1);
    ResumeDispatcher disp;
    // Use extra table to force compaction interleaving
    if (churnMode && i % 2 == 0) {
      // Acquire temp fd that will be removed to cause compaction
      int tmp[2];
      ::socketpair(AF_UNIX, SOCK_STREAM, 0, tmp);
      int tmpFd = tmp[0];
      int tmpPeer = tmp[1];
      tableB.acquire(tmpFd, POLLIN);
      int idx = tableB.indexOf(fdsB[i]);
      std::thread worker([&]() {
  ...
  // Group 3: slot table ownership with churn
